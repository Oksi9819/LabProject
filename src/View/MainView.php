...
class MainView extends BasicView
{
    ...
    {
        $title = $info[0]['page_name']; 
        echo $this->template->run("main.contacts", ['categories'=>$this->categories,'SESSION'=>$this->session, 'BASEPATH'=>BASEPATH, 'title'=>$title, 'info'=>$info[0]]);
    }

    public function showContactForm(array $info, array $values)
    {
        $title = $info[0]['page_name'];  
        echo $this->template->run("main.contacts", ['categories'=>$this->categories,'SESSION'=>$this->session, 'BASEPATH'=>BASEPATH, 'title'=>$title, 'info'=>$info[0], 'values'=>$values]);   
    }
}